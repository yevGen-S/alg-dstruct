...
node *insert_node(node *tree, int value){// вставка узла
    if (tree == NULL) {
        return create_node(value); // если дерево пусто, то создаем первую 2-3-вершину
    }
...
void print_2_3tree(node* tree, int y){
    if (tree != NULL){
        print_2_3tree(tree->first , y + 2);
        gotoxy(y);
        for(int i = 0; i < tree->size; i++){
            if(i == tree->size - 1){
                printf("%d", tree->key[i]);
        ...
        x += 5;
        print_2_3tree(tree->second, y + 2);
        x += 5;
        print_2_3tree(tree->third, y + 2);
    }
}
